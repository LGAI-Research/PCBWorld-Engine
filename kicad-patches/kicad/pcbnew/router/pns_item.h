 ...
 *
 * Keys, in order — GEOMETRY first, so two worlds holding the same copper order it
 * identically however they were built and whatever UUIDs the board file carries:
 *   kind → layer start → layer end → anchors → anchor count
 *   → parent UUID (cached, see ITEM::ParentUuid — nil, i.e. parentless, sorts first)
 *   → Serial().
 * The UUID/Serial tail only separates items whose copper is literally identical, so
 * the routing decision is a function of the observed geometry (state-determinism);
 * see NODE::CanonicalizeOrder and PNS_RL_ROUTER::startRoute.
 *
