...
int canonicalItemCompare( const ITEM* aA, const ITEM* aB )
{
    ...
    //
    // GEOMETRY FIRST (kind → layers → anchors), parent UUID and Serial() only as the
    // final tie-breaks for items whose copper is literally identical. Two worlds that
    // hold the same geometry therefore order it identically, whatever UUIDs the board
    // file carries and whatever the UUID generator position was when routed items were
    // minted — the routing decision becomes a function of the observed state.
    //
    // (Previously the parent UUID was the FIRST key. That made the order depend on
    // values the agent cannot observe: a re-rendered file with fresh pad UUIDs, or a
    // one-UUID shift of the generator stream, flipped ~10% of walkaround demos with
    // identical geometry — measured 2026-09-02.)
    //
    // Read the parent UUID from ITEM::ParentUuid(), the copy cached at SetParent() time,
    // NOT through Parent(): NODE::Commit sorts its `removed` set AFTER
    // ROUTER::CommitRouting has called m_iface->Commit(), which already deleted those
    // items' board parents, so `Parent()->m_Uuid` there is a use-after-free.
    ...
    // Identical geometry. Distinct parents (e.g. two pads of different footprints at
    // the same spot, or several SOLIDs synced from one PCB_SHAPE / zone triangles
    // sharing a position) still order by a content-derived key: the parent's board
    // UUID (nil for parentless, i.e. not-yet-committed, items — they sort first).
    ...
    // Exact-duplicate temporaries (same kind/layer/anchors/parent): break the tie by
    ...
    // object — any two distinct items must differ somewhere in the (kind, layer,
    // anchors, parent UUID, Serial) key. Firing means Serial uniqueness
