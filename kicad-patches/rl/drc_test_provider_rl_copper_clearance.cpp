...
/*
 * ============================================================================
 *  RL FORK of drc_test_provider_copper_clearance.cpp  (tag: _rl_)
 * ============================================================================
 *  Drop-in replacement for the stock copper-clearance provider, compiled into
 *  kicad_rl_router INSTEAD of the original (see kicad-patches/rl/CMakeLists.txt
 *  DRC_SRCS). The stock file is left pristine so the original DRC can be kept and
 *  compared (swap the DRC_SRCS entry back to compare). Same class name and self-
 *  registration, so it is a transparent substitute. Two RL differences from the
 *  stock provider:
 *
 *   1. Event-driven wait in testTrackClearances(): the main thread waits with
 *      wait_for_tasks_duration(250ms), returning as soon as the parallel work
 *      finishes — 250 ms is only the progress-report cadence (the pattern every
 *      other loop in this file uses).
 *
 *   2. Incremental clearance scope (drc/drc_rl_scope.h): when DRC_RL::GetClearanceScope()
 *      is non-null, the outer track loop is restricted to those items (each still tested
 *      vs the full RTree → identical results for any pair involving a scoped item) and
 *      the pad/graphic/zone sub-tests are skipped (retained by the caller). This is the
 *      "recompute only the changed items" half of an incremental DRC pass.
 * ============================================================================
 */
...
// RL incremental-DRC clearance scope is read via DRC_RL::GetClearanceScope()
// (declared in drc/drc_rl_scope.h, defined in drc_rl_scope.cpp). Keeping the definition
// in its own TU lets the stock provider be swapped back in for comparison without a link
// error from the scope symbols PNS_RL_ROUTER::runDRCEngine() always references.
    ...
    if( constraint.GetSeverity() != RPT_SEVERITY_IGNORE && clearance > 0 )
    {
        ...
        if( itemShape->Collide( otherShape, clearance - m_drcEpsilon, &actual, &pos ) )
        {
            ...
            else if( testClearance )
            {
                ...
                ReportAndShowPathCuToCu( drcItem, pos, layer, item, other, layer, actual );
                has_error = true;
        ...
        for( size_t ii = 0; ii < 2; ++ii )
        {
            ...
            if( constraint.GetSeverity() != RPT_SEVERITY_IGNORE )
            {
                                          ...
                                          &actual, &pos ) )
                {
                    ...
                    ReportAndShowPathCuToCu( drcItem, pos, layer, a[ii], b[ii], layer, actual );
                    return false;
                }
            }
        }
                                                              ...
                                                              PCB_LAYER_ID aLayer )
{
    ...
    if( constraint.GetSeverity() != RPT_SEVERITY_IGNORE && clearance > 0 )
    {
                                      ...
                                      std::max( 0, clearance - m_drcEpsilon ), &actual, &pos ) )
        {
            ...
            drcItem->SetViolatingRule( constraint.GetParentRule() );
            ReportAndShowPathCuToCu( drcItem, pos, aLayer, aItem, aZone, aLayer, actual );
        }
    }
                                                                      ...
                                                                      ZONE* aZone )
{
    ...
    if( constraint.GetSeverity() != RPT_SEVERITY_IGNORE && clearance >= 0 )
    {
                                      ...
                                      std::max( 0, clearance - m_drcEpsilon ), &actual, &pos ) )
        {
            ...
            drcItem->SetViolatingRule( constraint.GetParentRule() );
            ReportAndShowPathCuToCu( drcItem, pos, layer, aText, aZone, layer, actual );
        }
    }
}
...
void DRC_TEST_PROVIDER_COPPER_CLEARANCE::testTrackClearances()
{
    ...
    // RL incremental DRC: optionally restrict the outer loop to a scope set (each
    // scoped item is still tested against the full RTree, so results are identical
    // to a full pass for any pair involving a scoped item). Default: all tracks.
    std::vector<PCB_TRACK*> scopeTracks;
    if( const std::vector<BOARD_ITEM*>* scope = DRC_RL::GetClearanceScope() )
    {
        for( BOARD_ITEM* it : *scope )
            if( PCB_TRACK* t = dynamic_cast<PCB_TRACK*>( it ) )
                scopeTracks.push_back( t );
    }
    else
    {
    ...
    auto testTrack = [&]( const int start_idx, const int end_idx )
    {
        ...
        {
            PCB_TRACK* track = scopeTracks[trackIdx];
    ...
    tp.push_loop( scopeTracks.size(), testTrack );

    // Event-driven wait: wait_for_tasks_duration() returns as soon as the loop
    // finishes; the 250 ms is only the progress-report cadence. Every other
    // provider loop in this file also waits event-driven (see testZonesToZones).
    while( true )
    {
        ...
        if( constraint.GetSeverity() != RPT_SEVERITY_IGNORE && clearance > 0 )
        {
            if( padShape->Collide( otherShape.get(), sub_e( clearance ), &actual, &pos ) )
            {
                ...
                else if( testClearance )
                {
                    ...
                    drcItem->SetItems( pad, other );
                    drcItem->SetViolatingRule( constraint.GetParentRule() );
                    ReportAndShowPathCuToCu( drcItem, pos, aLayer, pad, other, aLayer, actual );
                    has_error = true;
                    testHoles = false;  // No need for multiple violations
                }
            }
    ...
    auto doTestHole =
            [&]( BOARD_ITEM* item, SHAPE* shape, BOARD_ITEM* otherItem, SHAPE* otherShape, int clearance )
            {
                if( shape->Collide( otherShape, sub_e( clearance ), &actual, &pos ) )
                {
                    ...
                    drcItem->SetViolatingRule( constraint.GetParentRule() );
                    ReportAndShowPathCuToCu( drcItem, pos, aLayer, item, otherItem, aLayer, actual );
                    has_error = true;
                    testHoles = false;  // No need for multiple violations
                }
            };
...
{
    // RL incremental DRC: pads don't move during routing, so with a clearance scope
    // set we skip pad-pad clearance (retained); pad-vs-changed-track clearance is
    // covered by the scoped testTrackClearances (track outer, pad RTree neighbor).
    if( DRC_RL::GetClearanceScope() )
        return;
...
{
    // RL incremental DRC: board graphics don't change during routing — skip when a
    // clearance scope is set (graphic-vs-changed-track is covered by testTrackClearances).
    if( DRC_RL::GetClearanceScope() )
        return;
...
{
    // RL incremental DRC: zones are excluded upfront (incremental falls back to full
    // when the board has zones), so this is belt-and-suspenders.
    if( DRC_RL::GetClearanceScope() )
        return;
    ...
    auto reportZoneZoneViolation =
                    ...
                    PCB_LAYER_ID layer ) -> void
            {
                ...
                else
                {
                    ...
                    drcItem->SetItems( zoneA, zoneB );
                    drcItem->SetViolatingRule( constraint.GetParentRule() );
                    ReportAndShowPathCuToCu( drcItem, pt, layer, zoneA, zoneB, layer, actual );
                }
            };
