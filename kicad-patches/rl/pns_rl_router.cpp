...
bool PNS_RL_ROUTER::startRoute( double x_mm, double y_mm, int boardLayer )
{
    ...
    // State-determinism: re-derive the root world's joint-link order and INDEX from
    // item content (geometry-first canonical key) before every session, so the
    // routing decision depends only on the copper present — not on the order it was
    // committed, restored or loaded in. Cost: one sort + R-tree rebuild per session
    // start (measured on the largest d3c board, ~12k items: see ENGINE.md).
    m_router->GetWorld()->CanonicalizeOrder();
...
// Board graphics that can carry the outline: the board-level drawings plus every footprint's graphical items.
// KiCad's own outline builder (BOARD::GetBoardPolygonOutlines / convert_shape_list_to_polygon), bbox and the PNS world
// all include footprint-owned Edge_Cuts shapes; some real boards draw the whole outline inside a footprint.
static std::vector<BOARD_ITEM*> outlineShapeItems( const BOARD* aBoard )
{
    std::vector<BOARD_ITEM*> items( aBoard->Drawings().begin(), aBoard->Drawings().end() );
...
std::vector<RLBoardEdge> PNS_RL_ROUTER::getBoardOutline() const
{
    ...
    for( BOARD_ITEM* item : outlineShapeItems( m_board ) )
    {
                                          ...
                                          const std::vector<std::array<long long, 7>>& aNewArcs )
{
    ...
    std::vector<BOARD_ITEM*> drawings = outlineShapeItems( m_board );
    ...
    {
        if( FOOTPRINT* owner = shape->GetParentFootprint() )
            owner->Remove( shape, REMOVE_MODE::NORMAL );   // footprint-owned outline shape: remove it from its footprint
        else
