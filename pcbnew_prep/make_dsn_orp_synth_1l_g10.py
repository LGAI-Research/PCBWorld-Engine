#!/usr/bin/python3
"""
synth_1L_grid10_5net_v02/{split}/board_XXXXX.kicad_pcb + .kicad_pro →
synth_1L_grid10_5net_v02_dsn/{split}/board_XXXXX_unrouted.dsn + board_XXXXX.orp
...
if not _SRC or not _DST:
    raise SystemExit(
        ...
        'targets variant synth_1L_grid10_5net_v02; the env vars should match that variant).'
    )
SRC_ROOT = Path(_SRC)
DST_ROOT = Path(_DST)
...
def process_board(pcb_path, pro_path, dst_dir, board_stem, verbose=False):
    ...
    try:
        ...
        # These are 1-layer boards (the stackup nominally declares F.Cu + B.Cu,
        # but routing is single-layer). Force a single copper layer before the
        # Specctra export so FreeRouting cannot escape to B.Cu or place vias —
        # matching the model's default_no_via training mask.
