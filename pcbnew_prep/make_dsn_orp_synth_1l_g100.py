#!/usr/bin/python3
"""
synth_1L_grid100_5net_v02/{split}/board_XXXXX.kicad_pcb + .kicad_pro →
synth_1L_grid100_5net_v02_dsn/{split}/board_XXXXX_unrouted.dsn + board_XXXXX.orp
...
if not _SRC or not _DST:
    raise SystemExit(
        ...
        'targets variant synth_1L_grid100_5net_v02; the env vars should match that variant).'
    )
SRC_ROOT = Path(_SRC)
DST_ROOT = Path(_DST)
